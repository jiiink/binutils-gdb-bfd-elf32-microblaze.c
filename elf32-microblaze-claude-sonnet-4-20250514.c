				  ...
				  bfd_reloc_code_real_type code)
{
  static const struct {
    ...
    enum elf_microblaze_reloc_type microblaze_val;
  } reloc_map[] = {
      ...
      if (reloc_map[i].bfd_val == code)
        {
          ...
          return microblaze_elf_howto_table[(int)reloc_map[i].microblaze_val];
        }
    }
...
microblaze_elf_link_hash_table_create (bfd *abfd)
{
  ...
  ret = (struct elf32_mb_link_hash_table *) bfd_zmalloc (sizeof (struct elf32_mb_link_hash_table));
  if (ret == NULL)
    return NULL;
...
microblaze_elf_final_sdp (struct bfd_link_info *info)
{
  ...
  if (h && h->type == bfd_link_hash_defined && h->u.def.section && h->u.def.section->output_section)
					  ...
					  bfd_vma addend)
{
  ...
  reloc_location = sreloc->contents + reloc_index * sizeof (Elf32_External_Rela);
  bfd_elf32_swap_reloca_out (output_bfd, &rel, reloc_location);
}
				 ...
				 asection **local_sections)
{
  ...
  for (; rel < relend; rel++)
      ...
      else
	{
	  ...
	  switch ((int) r_type)
	    ...
	    case (int) R_MICROBLAZE_SRO32 :
	      if (sec && handle_sro32_relocation(sec, &relocation, &ret, &r, howto, input_bfd, input_section, contents, offset, addend, sym_name, r_type))
		continue;
	      break;

	    case (int) R_MICROBLAZE_SRW32 :
	      if (sec && handle_srw32_relocation(sec, &relocation, &ret, &r, howto, input_bfd, input_section, contents, offset, addend, sym_name, r_type))
		continue;
	      break;
	    ...
	    case (int) R_MICROBLAZE_PLT_64:
	      handle_plt64_relocation(htab, h, &relocation, unresolved_reloc, 
				      input_bfd, input_section, contents, offset, endian);
	      break;
	    ...
	    case (int) R_MICROBLAZE_GOT_64:
	      handle_got64_relocation(htab, info, h, local_got_offsets, r_symndx, 
				      tls_type, relocation, addend, resolved_to_zero, 
				      input_bfd, contents, offset, endian, output_bfd);
	      unresolved_reloc = false;
	      break;

	    case (int) R_MICROBLAZE_GOTOFF_64:
	      handle_gotoff64_relocation(htab, relocation, addend, input_bfd, contents, offset, endian);
	      break;
	    ...
	    case (int) R_MICROBLAZE_32:
	      if (handle_standard_relocation(info, input_bfd, input_section, contents,
					     rel, offset, relocation, addend, h, sym,
					     sec, r_type, howto, sreloc, output_bfd,
					     endian, resolved_to_zero, symtab_hdr))
		break;
	      break;
    ...
    check_reloc:
      if (r != bfd_reloc_ok)
	{
	  const char *name = get_symbol_name(h, input_bfd, symtab_hdr, sym, sec);
...
static bool
handle_sro32_relocation(asection *sec, bfd_vma *relocation, bool *ret, 
			bfd_reloc_status_type *r, reloc_howto_type *howto,
			bfd *input_bfd, asection *input_section, bfd_byte *contents,
			bfd_vma offset, bfd_vma addend, const char *sym_name, int r_type)
{
  ...
  if (strcmp (name, ".sdata2") == 0 || strcmp (name, ".sbss2") == 0)
	...
	(_("%pB: the target (%s) of an %s relocation"
	 ...
	 input_bfd, sym_name,
	 microblaze_elf_howto_table[(int) r_type]->name, sec);
      *ret = false;
      return true;
    }
  return false;
...
static bool
handle_srw32_relocation(asection *sec, bfd_vma *relocation, bool *ret,
			bfd_reloc_status_type *r, reloc_howto_type *howto,
			bfd *input_bfd, asection *input_section, bfd_byte *contents,
			bfd_vma offset, bfd_vma addend, const char *sym_name, int r_type)
{
			...
			int endian, bfd *output_bfd)
{
  ...
  process_got_entry(htab, info, output_bfd, off, off2, tls_type, 
		    static_value, need_relocs, indx, offp);
...
static void
process_got_entry(struct elf32_mb_link_hash_table *htab,
		  struct bfd_link_info *info, bfd *output_bfd,
		  bfd_vma off, bfd_vma off2, unsigned int tls_type,
		  bfd_vma static_value, bool need_relocs,
		  unsigned long indx, bfd_vma *offp)
{
  if (!((*offp) & 1))
    {
      ...
      else if (IS_TLS_GD(tls_type))
	{
	  if (!need_relocs)
	    ...
	    microblaze_elf_output_dynamic_relocation
	      (output_bfd, htab->elf.srelgot, htab->elf.srelgot->reloc_count++,
	       indx, R_MICROBLAZE_TLSDTPMOD32, got_offset, 
	       indx ? 0 : static_value);
	}
      ...
      else if (IS_TLS_GD(tls_type))
	{
	  *offp |= 1;
	  ...
	  if (need_relocs)
	    microblaze_elf_output_dynamic_relocation
	      (output_bfd, htab->elf.srelgot, htab->elf.srelgot->reloc_count++,
	       indx, R_MICROBLAZE_TLSDTPREL32, got_offset,
	       indx ? 0 : static_value);
	  else
...
static void
handle_gotoff64_relocation(struct elf32_mb_link_hash_table *htab,
...
static bool
handle_standard_relocation(struct bfd_link_info *info, bfd *input_bfd,
			   asection *input_section, bfd_byte *contents,
			   Elf_Internal_Rela *rel, bfd_vma offset,
			   bfd_vma relocation, bfd_vma addend,
			   struct elf_link_hash_entry *h,
			   Elf_Internal_Sym *sym, asection *sec,
			   int r_type, reloc_howto_type *howto,
			   asection *sreloc, bfd *output_bfd,
			   int endian, bool resolved_to_zero,
			   Elf_Internal_Shdr *symtab_hdr)
{
  ...
  if (r_symndx == STN_UNDEF || (input_section->flags & SEC_ALLOC) == 0)
    {
      write_standard_relocation(relocation, addend, r_type, input_bfd,
				contents, offset, endian, input_section);
      return true;
    }

  if (should_create_dynamic_reloc(info, h, resolved_to_zero, howto))
    {
      return create_dynamic_relocation(info, input_bfd, input_section, rel,
				       offset, sreloc, h, addend, r_type,
				       output_bfd);
    }
  else
    {
...
static void
write_standard_relocation(bfd_vma relocation, bfd_vma addend, int r_type,
			  bfd *input_bfd, bfd_byte *contents, bfd_vma offset,
			  int endian, asection *input_section)
{
  relocation += addend;
			    ...
			    bool resolved_to_zero, reloc_howto_type *howto)
{
  return ((bfd_link_pic (info)
	  ...
	  || (!bfd_link_pic (info) && h != NULL && h->dynindx != -1
	      && !h->non_got_ref
...
static bool
create_dynamic_relocation(struct bfd_link_info *info, bfd *input_bfd,
			  asection *input_section, Elf_Internal_Rela *rel,
			  bfd_vma offset, asection *sreloc,
			  struct elf_link_hash_entry *h, bfd_vma addend,
			  int r_type, bfd *output_bfd)
{
  ...
  else if (h != NULL && ((!info->symbolic && h->dynindx != -1)
			 || !h->def_regular))
    {
  ...
  for (i = 0; i < sdata->relax_count; i++)
    {
      if (end <= sdata->relax[i].addr)
        break;
      if (start != end && start > sdata->relax[i].addr)
        continue;
...
{
    const unsigned long imm_mask = 0x0000ffff;
  ...
  if (sdata->relax_count > 0) {
    ...
    for (o = abfd->sections; o != NULL; o = o->next) {
      ...
      for (irelscan = irelocs; irelscan < irelscanend; irelscan++) {
        int scan_type = ELF32_R_TYPE (irelscan->r_info);
        
        if (scan_type == R_MICROBLAZE_32 || scan_type == R_MICROBLAZE_32_NONE) {
        ...
        } else if (scan_type == R_MICROBLAZE_32_SYM_OP_SYM) {
          isym = isymbuf + ELF32_R_SYM (irelscan->r_info);
        ...
        } else if (scan_type == R_MICROBLAZE_32_PCREL_LO ||
                   scan_type == R_MICROBLAZE_32_LO ||
                   scan_type == R_MICROBLAZE_TEXTREL_32_LO) {
          isym = isymbuf + ELF32_R_SYM (irelscan->r_info);
        ...
        } else if (scan_type == R_MICROBLAZE_64 ||
                   scan_type == R_MICROBLAZE_TEXTREL_64) {
          isym = isymbuf + ELF32_R_SYM (irelscan->r_info);
        ...
        } else if (scan_type == R_MICROBLAZE_64_PCREL) {
          isym = isymbuf + ELF32_R_SYM (irelscan->r_info);
		       ...
		       unsigned int tls_type)
{
  ...
  if (abfd == NULL || symtab_hdr == NULL || r_symndx >= symtab_hdr->sh_info)
    return false;
  ...
  if (local_got_refcounts == NULL)
    {
      ...
      if (size == 0 || size > SIZE_MAX / (sizeof (*local_got_refcounts) + sizeof (*local_got_tls_masks)))
        return false;
  ...
  for (rel = relocs; rel < rel_end; rel++)
    {
      ...
      switch (r_type)
        ...
        case R_MICROBLAZE_TLSGD:
          tls_type |= (TLS_TLS | TLS_GD);
          if (!handle_tls_relocation(abfd, info, sec, htab, h, r_symndx, symtab_hdr, tls_type))
            return false;
          break;
        ...
        case R_MICROBLAZE_GOT_64:
          if (!handle_got_relocation(abfd, info, htab, h, r_symndx, symtab_hdr, tls_type))
            return false;
          break;
        ...
        case R_MICROBLAZE_32:
          if (!handle_standard_relocation(abfd, info, sec, htab, h, rel, r_type, &sreloc))
            return false;
          break;
        }
    }
...
static bool
handle_tls_relocation(bfd *abfd, struct bfd_link_info *info,
                      asection *sec, struct elf32_mb_link_hash_table *htab,
                      struct elf_link_hash_entry *h,
                      unsigned long r_symndx,
                      Elf_Internal_Shdr *symtab_hdr,
                      unsigned char tls_type)
{
  sec->has_tls_reloc = 1;
  return handle_got_relocation(abfd, info, htab, h, r_symndx, symtab_hdr, tls_type);
}

static bool
needs_dynamic_relocation(struct bfd_link_info *info, asection *sec,
                        struct elf_link_hash_entry *h, unsigned int r_type)
{
  return (bfd_link_pic(info) &&
          (sec->flags & SEC_ALLOC) != 0 &&
          (r_type != R_MICROBLAZE_64_PCREL ||
           (h != NULL && (!info->symbolic ||
            h->root.type == bfd_link_hash_defweak ||
            !h->def_regular)))) ||
         (!bfd_link_pic(info) &&
          (sec->flags & SEC_ALLOC) != 0 &&
...
create_dynamic_reloc_section(struct elf32_mb_link_hash_table *htab,
                             asection *sec, bfd *abfd, asection **sreloc)
{
  if (*sreloc != NULL)
    return true;
...
get_relocs_head(struct elf32_mb_link_hash_table *htab,
                struct elf_link_hash_entry *h,
                bfd *abfd, unsigned long r_symndx)
{
  if (h != NULL)
    return &h->dyn_relocs;
  ...
  return (struct elf_dyn_relocs **) &elf_section_data(s)->local_dynrel;
}

static bool
add_dynamic_reloc(struct elf32_mb_link_hash_table *htab,
                  struct elf_dyn_relocs **head,
                  asection *sec, unsigned int r_type)
{
...
static bool
handle_standard_relocation(bfd *abfd, struct bfd_link_info *info,
                          asection *sec, struct elf32_mb_link_hash_table *htab,
                          struct elf_link_hash_entry *h,
                          const Elf_Internal_Rela *rel,
                          unsigned int r_type, asection **sreloc)
{
  unsigned long r_symndx = ELF32_R_SYM(rel->r_info);
  ...
  if (!needs_dynamic_relocation(info, sec, h, r_type))
    return true;
    
  if (!create_dynamic_reloc_section(htab, sec, abfd, sreloc))
    return false;
    
  struct elf_dyn_relocs **head = get_relocs_head(htab, h, abfd, r_symndx);
  if (head == NULL)
    return false;
    
  return add_dynamic_reloc(htab, head, sec, r_type);
}
...
static bool ensure_dynamic_symbol(struct bfd_link_info *info, struct elf_link_hash_entry *h)
{
...
static void handle_plt_allocation(struct elf_link_hash_entry *h, struct elf32_mb_link_hash_table *htab, struct bfd_link_info *info)
{
...
static unsigned int calculate_got_size(struct elf32_mb_link_hash_entry *eh, struct elf32_mb_link_hash_table *htab)
{
    unsigned int need = 0;
...
static void handle_got_allocation(struct elf_link_hash_entry *h, struct elf32_mb_link_hash_table *htab)
{
...
static void handle_shared_relocations(struct elf_link_hash_entry *h, struct bfd_link_info *info)
{
...
static bool handle_non_shared_relocations(struct elf_link_hash_entry *h, struct elf32_mb_link_hash_table *htab, struct bfd_link_info *info)
{
    if (h->non_got_ref || 
        (!h->def_dynamic || h->def_regular) &&
...
static void allocate_relocation_space(struct elf_link_hash_entry *h)
{
    struct elf_dyn_relocs *p;
...
static bool allocate_dynrelocs(struct elf_link_hash_entry *h, void *dat)
{
    ...
    if (htab->elf.dynamic_sections_created && h->plt.refcount > 0) {
        ...
        handle_plt_allocation(h, htab, info);
    } else {
        h->plt.offset = (bfd_vma) -1;
        h->needs_plt = 0;
    }
    ...
    if (bfd_link_pic(info)) {
        handle_shared_relocations(h, info);
    } else {
        if (!handle_non_shared_relocations(h, htab, info)) {
            return false;
        }
    }
...
static bool
process_copy_reloc(bfd *output_bfd,
                   struct elf_link_hash_entry *h,
                   struct elf32_mb_link_hash_table *htab)
{
  asection *s;
					...
					struct bfd_link_info *info)
{
  ...
  if (htab->elf.dynamic_sections_created)
    {
      if (!microblaze_elf_process_dynamic_entries(output_bfd, dynobj, sdyn, htab))
        return false;

      if (!microblaze_elf_setup_plt(output_bfd, htab, sdyn))
        return false;
    }

  microblaze_elf_setup_got(output_bfd, htab, sdyn);
  return true;
}

static bool
microblaze_elf_process_dynamic_entries(bfd *output_bfd, bfd *dynobj, 
                                     asection *sdyn,
                                     struct elf32_mb_link_hash_table *htab)
{
  ...
  for (; dyncon < dynconend; dyncon++)
    {
      ...
      s = microblaze_elf_get_section_for_dyn_tag(dyn.d_tag, htab, &use_size);
...
static asection *
microblaze_elf_get_section_for_dyn_tag(bfd_vma tag,
                                      struct elf32_mb_link_hash_table *htab,
                                      bool *use_size)
{
  *use_size = false;
  ...
  switch (tag)
    {
    case DT_PLTGOT:
      return htab->elf.sgotplt;

    case DT_PLTRELSZ:
      *use_size = true;
      return htab->elf.srelplt;
...
static bool
microblaze_elf_setup_plt(bfd *output_bfd, struct elf32_mb_link_hash_table *htab,
                        asection *sdyn)
{
...
static void
microblaze_elf_setup_got(bfd *output_bfd, struct elf32_mb_link_hash_table *htab,
                        asection *sdyn)
{
