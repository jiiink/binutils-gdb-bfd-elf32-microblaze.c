...
microblaze_elf_reloc_type_lookup(bfd *abfd ATTRIBUTE_UNUSED, bfd_reloc_code_real_type code)
{
    static const struct {
        bfd_reloc_code_real_type code;
        enum elf_microblaze_reloc_type reloc;
    } code_reloc_map[] = {
    ...
    size_t i;
    for (i = 0; i < sizeof(code_reloc_map) / sizeof(code_reloc_map[0]); ++i) {
        if (code_reloc_map[i].code == code) {
            microblaze_reloc = code_reloc_map[i].reloc;
            break;
        }
    }
    if (i == sizeof(code_reloc_map) / sizeof(code_reloc_map[0]))
        return NULL;
...
static bool microblaze_elf_info_to_howto(bfd *abfd, arelent *cache_ptr, Elf_Internal_Rela *dst)
{
    ...
    if (r_type >= R_MICROBLAZE_max || !microblaze_elf_howto_table[r_type]) {
...
static bool microblaze_elf_is_local_label_name(bfd *abfd, const char *name)
{
    if (!name || !abfd)
        return false;
...
static void microblaze_elf_output_dynamic_relocation(bfd *output_bfd, asection *sreloc, unsigned long reloc_index, unsigned long indx, int r_type, bfd_vma offset, bfd_vma addend) {
    ...
    size_t offset_in_section = reloc_index * sizeof(Elf32_External_Rela);
    if (offset_in_section + sizeof(Elf32_External_Rela) > sreloc->size)
        return;

    bfd_elf32_swap_reloca_out(output_bfd, &rel, sreloc->contents + offset_in_section);
}
...
static int microblaze_elf_relocate_section(bfd *output_bfd,
                             ...
                             asection **local_sections)
{
    ...
    for (; rel < relend; rel++)
    {
        ...
        if (r_type < 0 || r_type >= (int)R_MICROBLAZE_max) {
            _bfd_error_handler(_("%pB: unsupported relocation type %#x"), input_bfd, r_type);
            bfd_set_error(bfd_error_bad_value);
            ret = false;
            continue;
        }
        ...
        if (bfd_link_relocatable(info)) {
            ...
            addend += (sec ? sec->output_offset : 0) + sym->st_value;
            ...
            resolved_to_zero = (h && UNDEFWEAK_NO_DYNAMIC_RELOC(info, h));

            switch (r_type)
            {
                case R_MICROBLAZE_SRO32:
                {
                    const char *name = sec ? bfd_section_name(sec) : NULL;
                    if (sec && name &&
                        (!strcmp(name, ".sdata2") || !strcmp(name, ".sbss2"))) {
                ...
                case R_MICROBLAZE_SRW32:
                {
                    const char *name = sec ? bfd_section_name(sec) : NULL;
                    if (sec && name &&
                        (!strcmp(name, ".sdata") || !strcmp(name, ".sbss"))) {
                ...
                {
                    bfd_vma immediate;
                    if (htab->elf.splt && h && h->plt.offset != (bfd_vma)-1) {
...
dogot:
                case R_MICROBLAZE_GOT_64:
                {
                    ...
                    bfd_vma static_value = 0;
                    bool need_relocs = false, is_global = false;
                    ...
                    if (IS_TLS_LD(tls_type))
                        ...
                        if (htab->elf.sgotplt && h->got.offset != (bfd_vma)-1)
                            offp = &h->got.offset;
                        else
                            abort();
                    } else {
                    ...
                    off = (*offp) & ~1;
                    off2 = off;
                    if (IS_TLS_LD(tls_type) || IS_TLS_GD(tls_type)) off2 = off + 4;

                    if (h) {
                        bool dyn = elf_hash_table(info)->dynamic_sections_created;
                        is_global = WILL_CALL_FINISH_DYNAMIC_SYMBOL(dyn, bfd_link_pic(info), h)
                ...
                {
                    if (r_symndx == STN_UNDEF || !(input_section->flags & SEC_ALLOC)) {
                        relocation += addend;
                    ...
                    bool picrelocs =
                        (bfd_link_pic(info)
                         && (h == NULL
                             ...
                             || h->root.type != bfd_link_hash_undefweak)
                         && (!howto->pc_relative
                             || (h && h->dynindx != -1 && (!info->symbolic || !h->def_regular))))
                        || (!bfd_link_pic(info)
                            && h && h->dynindx != -1
                            && !h->non_got_ref
                    ...
                    if (picrelocs) {
                        ...
                        else if (h && ((!info->symbolic && h->dynindx != -1) || !h->def_regular)) {
            ...
            switch (r) {
                case bfd_reloc_overflow:
                    info->callbacks->reloc_overflow(info, h ? &h->root : NULL, name,
                                                    howto->name, 0, input_bfd, input_section, offset);
                    break;
...
static void microblaze_bfd_write_imm_value_32(bfd *abfd, bfd_byte *bfd_addr, bfd_vma val) {
    if (!abfd || !bfd_addr) {
        return;
    }
...
static void microblaze_bfd_write_imm_value_64(bfd *abfd, bfd_byte *bfd_addr, bfd_vma val) {
    if (!abfd || !bfd_addr)
        return;

    unsigned long instr;
    bfd_vma parts[2] = { (val >> 16) & 0xFFFF, val & 0xFFFF };

    for (int i = 0; i < 2; ++i) {
        bfd_byte *addr = bfd_addr + (i * INST_WORD_SIZE);
        instr = bfd_get_32(abfd, addr);
        instr = (instr & ~0xFFFFUL) | parts[i];
        bfd_put_32(abfd, instr, addr);
    }
}
...
static bool microblaze_elf_relax_section(bfd *abfd, asection *sec, struct bfd_link_info *link_info, bool *again)
{
    ...
    Elf_Internal_Rela *internal_relocs = NULL, *free_relocs = NULL;
    bfd_byte *contents = NULL, *free_contents = NULL;
    size_t i, sym_index, symcount, offset;
    bfd_vma src, dest;
    asection *o;
    struct elf_link_hash_entry *sym_hash;
    Elf_Internal_Sym *isymbuf, *isymend, *isym;
    unsigned int shndx;
    struct _microblaze_elf_section_data *sdata;
    ...
    *again = false;
    ...
    for (irel = internal_relocs; irel < irelend; irel++) {
        bfd_vma symval;
        int type = ELF32_R_TYPE(irel->r_info);
        if (type != (int)R_MICROBLAZE_64_PCREL
            && type != (int)R_MICROBLAZE_64
            && type != (int)R_MICROBLAZE_TEXTREL_64)
            continue;
        ...
        else if (type == (int)R_MICROBLAZE_TEXTREL_64)
        ...
        if ((symval & 0xffff8000) == 0 || (symval & 0xffff8000) == 0xffff8000) {
            ...
            switch ((enum elf_microblaze_reloc_type)type) {
            case R_MICROBLAZE_64_PCREL:
    ...
    if (sdata->relax_count > 0) {
        ...
        for (o = abfd->sections; o != NULL; o = o->next) {
            ...
            for (irelscan = irelocs; irelscan < irelscanend; irelscan++) {
                int rtype = ELF32_R_TYPE(irelscan->r_info);
                if (rtype == (int)R_MICROBLAZE_32 || rtype == (int)R_MICROBLAZE_32_NONE) {
                    ...
                    } else if (rtype == (int)R_MICROBLAZE_32_SYM_OP_SYM) {
                        isym = isymbuf + ELF32_R_SYM(irelscan->r_info);
                        if (ocontents == NULL) {
                            ocontents = elf_section_data(o)->this_hdr.contents;
                            if (ocontents == NULL) {
                ...
                } else if (rtype == (int)R_MICROBLAZE_32_PCREL_LO ||
                           rtype == (int)R_MICROBLAZE_32_LO ||
                           rtype == (int)R_MICROBLAZE_TEXTREL_32_LO) {
                    isym = isymbuf + ELF32_R_SYM(irelscan->r_info);
                    if (isym->st_shndx == shndx && ELF32_ST_TYPE(isym->st_info) == STT_SECTION) {
                        bfd_vma immediate, target_address;
                        if (ocontents == NULL) {
                            ocontents = elf_section_data(o)->this_hdr.contents;
                            if (ocontents == NULL) {
                                if (o->rawsize == 0)
                ...
                if (rtype == (int)R_MICROBLAZE_64 || rtype == (int)R_MICROBLAZE_TEXTREL_64) {
                    isym = isymbuf + ELF32_R_SYM(irelscan->r_info);
                    if (isym->st_shndx == shndx && ELF32_ST_TYPE(isym->st_info) == STT_SECTION) {
                        if (ocontents == NULL) {
                            ocontents = elf_section_data(o)->this_hdr.contents;
                ...
                } else if (rtype == (int)R_MICROBLAZE_64_PCREL) {
                    isym = isymbuf + ELF32_R_SYM(irelscan->r_info);
                    if (isym->st_shndx == shndx && ELF32_ST_TYPE(isym->st_info) == STT_SECTION) {
                        bfd_vma immediate, target_address;
                        if (ocontents == NULL) {
...
static bool update_local_sym_info(bfd *abfd, Elf_Internal_Shdr *symtab_hdr, unsigned long r_symndx, unsigned int tls_type)
{
    bfd_signed_vma *local_got_refcounts = elf_local_got_refcounts(abfd);
    unsigned char *local_got_tls_masks;
    bfd_size_type count;

    if (!symtab_hdr || r_symndx >= symtab_hdr->sh_info)
        return false;

    count = symtab_hdr->sh_info;

    if (local_got_refcounts == NULL)
    {
        bfd_size_type size = count * sizeof(*local_got_refcounts) + count * sizeof(unsigned char);
    ...
    local_got_tls_masks = (unsigned char *)(local_got_refcounts + count);
...
static bool microblaze_elf_check_relocs(bfd *abfd, struct bfd_link_info *info, asection *sec, const Elf_Internal_Rela *relocs)
{
    ...
    for (rel = relocs; rel < rel_end; rel++) {
        ...
        case R_MICROBLAZE_32: {
            ...
            bool pc_reloc = r_type == R_MICROBLAZE_64_PCREL;
            bool alloc_flag = (sec->flags & SEC_ALLOC) != 0;
            bool pic = bfd_link_pic(info);

            bool need_copy =
                (pic && alloc_flag &&
                 (!pc_reloc || (h && (!info->symbolic ||
...
static void microblaze_elf_copy_indirect_symbol(struct bfd_link_info *info,
...
static bool microblaze_elf_late_size_sections(bfd *output_bfd ATTRIBUTE_UNUSED, struct bfd_link_info *info)
{
    ...
    for (bfd *ibfd = info->input_bfds; ibfd != NULL; ibfd = ibfd->link.next) {
        ...
        for (asection *s = ibfd->sections; s != NULL; s = s->next) {
            for (struct elf_dyn_relocs *p = (struct elf_dyn_relocs *) elf_section_data(s)->local_dynrel;
                 p != NULL;
                 p = p->next)
            {
...
static bool microblaze_elf_finish_dynamic_symbol(bfd *output_bfd,
    ...
    if (!htab || !output_bfd || !info || !h || !sym || !eh) {
        return false;
    }
    ...
    if (h->plt.offset != (bfd_vma)-1) {
        ...
        if (!splt || !srela || !sgotplt || h->dynindx == -1) {
            return false;
        }
        ...
        if (!bfd_link_pic(info)) {
            if (!sgotplt->output_section) return false;
    ...
    if (h->got.offset != (bfd_vma)-1 &&
        ...
        if (!sgot->output_section) return false;
        ...
        if (bfd_link_pic(info) &&
            ...
            if (!microblaze_elf_output_dynamic_relocation(output_bfd,
    ...
    if (h->needs_copy) {
        ...
        if (h->dynindx == -1 || !sec || !sec->output_section) {
            return false;
        }
        ...
        rela.r_addend = 0;
        if (sec == htab->elf.sdynrelro)
            s = htab->elf.sreldynrelro;
        else
            s = htab->elf.srelbss;
        if (!s || !s->contents) return false;
...
static bool microblaze_elf_finish_dynamic_sections(bfd *output_bfd, struct bfd_link_info *info)
{
    struct elf32_mb_link_hash_table *htab = elf32_mb_hash_table(info);
    if (!htab)
        return false;
    ...
    if (htab->elf.dynamic_sections_created) {
        ...
        if (splt->size > 0) {
            ...
            if (splt->output_section && splt->output_section != bfd_abs_section_ptr) {
    ...
    if (sgot && sgot->size > 0) {
        ...
        bfd_put_32(output_bfd, val, sgot->contents);
...
static bool microblaze_elf_add_symbol_hook(bfd *abfd,
                                           struct bfd_link_info *info,
                                           Elf_Internal_Sym *sym,
                                           ...
                                           bfd_vma *valp)
{
    ...
    {
        asection *section = bfd_make_section_old_way(abfd, ".sbss");
        if (!section)
            return false;
        if (!bfd_set_section_flags(section, SEC_IS_COMMON | SEC_SMALL_DATA))
            return false;
        *secp = section;
        *valp = sym->st_size;
    }
